...
export function generateReadme(config: GenerateRequestInput): string {
  // Escape any backticks in the name to prevent markdown injection
  const safeName = config.name.replace(/`/g, "\\`");
  const safeUrl = config.url.replace(/`/g, "\\`");

  return `# ${safeName} Desktop App

Generated with [Deskify](https://deskify.dev).

## About

This is an Electron desktop application that wraps **${safeUrl}** in a native window.
